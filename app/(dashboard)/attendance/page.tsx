"use client";

import React, { useEffect } from "react";
import { useAppSelector, useAppDispatch } from "@/redux/type";
import { setLoading } from "@/redux/features/attendance.slice";
import EmptyState from "@/components/organisms/states/empty-state/EmptyState";
import LoadingState from "@/components/organisms/states/loading-state/LoadingState";

export default function AttendancePage() {
  const dispatch = useAppDispatch();
  const { attendance, loading, error } = useAppSelector(
    (state) => state.attendance
  );

  useEffect(() => {
    // Simulate fetching data
    dispatch(setLoading(true));
    // When you have an API, replace this with actual fetch
    setTimeout(() => {
      dispatch(setLoading(false));
    }, 1000);
  }, [dispatch]);

  const handleAddAttendance = () => {
    console.log("Add new attendance record");
    // TODO: Navigate to add attendance form or open modal
  };

  if (loading) {
    return (
      <LoadingState
        title="Loading attendance..."
        subtitle="Please wait while we fetch your attendance data."
      />
    );
  }

  if (error) {
    return (
      <div className="text-center text-red-600 p-4">
        <p>Error: {error}</p>
      </div>
    );
  }

  if (attendance.length === 0) {
    return (
      <EmptyState
        title="No attendance records available"
        subtitle="You haven't added any attendance records yet. Start by creating your first attendance record."
        buttonLabel="Add Attendance"
        onButtonClick={handleAddAttendance}
        icon="document-add"
        showButton={false}
      />
    );
  }

  return (
    <div className="p-4">
      <h1 className="text-2xl font-bold mb-4">Attendance</h1>
      {/* TODO: Add attendance list/table here */}
      <div className="grid gap-4">
        {attendance.map((record: any, index: number) => (
          <div key={index} className="border p-4 rounded-lg">
            {/* Attendance record content */}
            <p>{JSON.stringify(record)}</p>
          </div>
        ))}
      </div>
    </div>
  );
}
